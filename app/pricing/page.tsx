...
export default function PricingPage() {
  ...
  useEffect(() => {
    const checkUserSubscription = async () => {
      ...
      if (user) {
        try {
          // Pass both user.id and user.email to checkSubscription for backup
  ...
  const handleSubscribe = async () => {
    ...
    // Enhanced validation for user ID and email
    if (!user.id || typeof user.id !== 'string' || user.id.trim() === '') {
      console.error('Invalid user ID:', user.id)
      alert('Your account information is incomplete. Please try logging out and back in.')
      return
    }
    
    if (!user.email || typeof user.email !== 'string' || !user.email.includes('@')) {
      console.error('Invalid email:', user.email)
      alert('Your account email is invalid. Please update your profile with a valid email.')
      return
    }
    
    // Log checkout attempt for debugging
    console.log('Creating checkout with user ID:', user.id, 'and email:', user.email)
