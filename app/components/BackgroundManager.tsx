...
import { Edit, Trash2, Upload, Check, X } from 'lucide-react';
...
import { DEFAULT_BACKGROUNDS, DEFAULT_BACKGROUND_ID, DefaultBackground } from '@/app/photo/constants/backgrounds';

export function BackgroundManager({ isOpen, onClose, user, currentBackground, onBackgroundChange }: BackgroundManagerProps) {
  ...
  useEffect(() => {
    if (currentBackground) {
      ...
      const defaultBg = DEFAULT_BACKGROUNDS.find(bg => bg.url === currentBackground);
      if (defaultBg) {
  ...
  }, [isOpen, loadCustomBackgrounds]);

  // Image compression function (similar to AccountDialog)
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      {/* Modal */}
      <div 
        className="relative w-full max-w-2xl mx-4 max-h-[80vh] overflow-hidden rounded-2xl"
        style={{
          backgroundColor: document.documentElement.getAttribute('data-theme') === 'dark' || 
            (document.documentElement.getAttribute('data-theme') === 'system' && 
             window.matchMedia('(prefers-color-scheme: dark)').matches) ? 'rgba(0, 0, 0, 0.8)' : 'rgba(255, 255, 255, 0.9)',
          backdropFilter: (window.innerWidth <= 768 || /^((?!chrome|android).)*safari/i.test(navigator.userAgent)) ? 'blur(20px)' : 'url(#glass-distortion) blur(20px)',
          WebkitBackdropFilter: (window.innerWidth <= 768 || /^((?!chrome|android).)*safari/i.test(navigator.userAgent)) ? 'blur(20px)' : 'url(#glass-distortion) blur(20px)',
          border: document.documentElement.getAttribute('data-theme') === 'dark' || 
            (document.documentElement.getAttribute('data-theme') === 'system' && 
             window.matchMedia('(prefers-color-scheme: dark)').matches) ? '1px solid rgba(255, 255, 255, 0.1)' : '1px solid rgba(255, 255, 255, 0.2)',
      ...
      >
        ...
        <div className="flex items-center justify-between p-4 border-b border-white/10">
          <h2 className="text-lg font-semibold text-white">Background Settings</h2>
          <button
            onClick={onClose}
            ...
            <div>
              ...
              <div className="mb-4">
                ...
                <button
                  ...
                  className="w-full py-3 px-4 rounded-lg border-2 border-dashed border-white/30 hover:border-white/50 transition-colors flex items-center justify-center gap-2 text-white/70 hover:text-white"
                >
                  <Upload size={20} />
              ...
              {isLoading ? (
                ...
                <div className="flex items-center justify-center py-8">
                  <div className="text-white/70">No custom photos yet. Upload some to get started!</div>
                </div>
              ) : (
                <div className="grid grid-cols-2 gap-4">
        ...
        <div className="flex items-center justify-end gap-3 p-4 border-t border-white/10">
          ...
          <button
            ...
            className="px-6 py-2 bg-blue-500 hover:bg-blue-600 disabled:bg-white/20 disabled:cursor-not-allowed text-white rounded-lg transition-colors"
          >
            Apply
          </button>
        </div>
