...
const Message = memo(function MessageComponent({
  ...
  const handleTouchStart = (e: React.TouchEvent) => {
    ...
    // 모바일에서만 터치 위치에 따라 드롭다운 위치 결정
    const touchY = e.touches[0].clientY;
    const viewportHeight = window.innerHeight;
    const dropdownHeight = 120; // 드롭다운 예상 높이
    const margin = 20; // 여백
    
    // 화면 상단 1/3 영역이거나 드롭다운이 화면을 벗어날 경우 아래로
    if (touchY < viewportHeight / 3 || touchY + dropdownHeight + margin > viewportHeight) {
                    ...
                    <div className="relative">
                      <div 
                        ...
                        style={{
                          ...
                          transform: longPressActive ? 'scale(1.05) translateY(-2px)' : 'scale(1) translateY(0)',
