  ...
  const handleAITouchStart = (e: React.TouchEvent) => {
    ...
    if (aiBubbleRef.current) {
      ...
      if (needsScaling) {
        ...
        // 축소가 필요하지 않은 경우에도 오버레이 메트릭스 생성
        // 원본 위치를 그대로 사용하되 약간의 여유 공간 추가
        setOverlayMetrics({
          scale: 1,
          originalRect: rect,
          overlayPosition: { 
            top: Math.max(0, rect.top - 8), // 상단 여유 공간
            left: rect.left 
          },
          needsScaling: false
        });
      ...
      // 1. 먼저 메시지 바로 아래에 메뉴를 배치해보기 (오버레이 기준)
    ...
    <div className={`message-group group animate-fade-in ${getMinHeight}`} id={message.id}>
      ...
      <div className={`flex ${isUser ? `justify-end` : `justify-start`} ${isUser ? 'mt-10 sm:mt-12 mb-0 sm:mb-10' : ''}`}>
        {isUser ? (
            ...
            <div className="relative">
              <div 
                ...
                style={{ 
                  ...
                  zIndex: longPressActive ? 10 : 'auto',
                  position: longPressActive ? 'relative' : 'static',
                  // 롱프레스 시 원본 버블 숨기기 (애니메이션 포함)
                  opacity: (overlayPhase === 'active') ? 0 : 1,
                  visibility: (overlayPhase === 'active') ? 'hidden' : 'visible',
                }}
          ...
          {/* 오버레이 렌더링: 모든 롱프레스에 적용 */}
          {longPressActive && overlayMetrics && isAssistant && overlayPhase !== 'idle' && createPortal(
            <div
