...
import { SquarePencil, Bookmark } from 'react-ios-icons'
...
import { getAdaptiveGlassStyleClean, getAdaptiveGlassStyleBlur, getAdaptiveGlassBackgroundColor } from '@/app/lib/adaptiveGlassStyle'
...
export function ChatSidebar({ user, toggleSidebar, isBookmarkMode = false }: ChatSidebarProps) {
  ...
  const [bookmarkSearchCache, setBookmarkSearchCache] = useState<Map<string, any[]>>(new Map())

  const [lastLoadedUserId, setLastLoadedUserId] = useState<string | null>(null)
  const [isChatsLoaded, setIsChatsLoaded] = useState(false)
  const [isBookmarksLoaded, setIsBookmarksLoaded] = useState(false)
  const [bookmarks, setBookmarks] = useState<any[]>([])
  const [bookmarkSearchResults, setBookmarkSearchResults] = useState<any[]>([])
  const lastLoadTimeRef = useRef<number>(0)
  const lastBookmarkLoadTimeRef = useRef<number>(0)
  ...
  useEffect(() => {
    ...
    // 모드 전환 시 스크롤을 최상단으로 이동 및 검색어 초기화
    ...
    if (lastLoadedUserId === user.id) {
      if (isBookmarkMode && isBookmarksLoaded) return;
      if (!isBookmarkMode && isChatsLoaded) return;
    }

    if (isBookmarkMode) {
      loadBookmarks(1, false, false);
    } else {
      loadChats(1, false, false);
    }
  }, [user?.id, isAnonymousUser, isBookmarkMode]);
    ...
    if (
      ...
      try {
        ...
        if (successful.length > 0) {
          ...
          // 북마크 모드일 때 북마크 리스트에서도 제거
          if (isBookmarkMode) {
            setBookmarks(prev => prev.filter(b => !successful.includes(b.id)))
            setBookmarkSearchResults(prev => prev.filter(b => !successful.includes(b.id)))
          }
        }
  ...
  const loadBookmarks = useCallback(async (page = 1, append = false, forceRefresh = false) => {
    if (!user || isAnonymousUser) return;
    
    const now = Date.now();
    const isCacheValid = now - lastBookmarkLoadTimeRef.current < CACHE_DURATION;
    
    if (!forceRefresh && page === 1 && !append && lastLoadedUserId === user.id && isCacheValid && isBookmarksLoaded) {
      return;
    }
    ...
    try {
      ...
      if (bookmarkData.length < CHATS_PER_PAGE) {
        setHasMoreBookmarks(false);
      } else {
        setHasMoreBookmarks(true);
      }

      // 2. 각 북마크에 대한 메시지 내용 및 세션 정보 가져오기 (sequential fetching like /bookmarks)
      const processedBookmarks = await Promise.all(
        bookmarkData.map(async (bookmark) => {
          ...
          const { data: chatSession } = await supabase
            .from('chat_sessions')
            .select('title, current_model')
            .eq('id', bookmark.chat_session_id)
            .single();

          return {
            id: bookmark.chat_session_id,
            bookmark_id: bookmark.id,
            message_id: bookmark.message_id,
            title: chatSession?.title || 'Untitled Chat',
            lastMessage: messageData?.content || '',
            lastMessageTime: new Date(bookmark.created_at).getTime(),
            current_model: chatSession?.current_model || messageData?.model,
      ...
      if (append) {
        ...
        setHasMoreBookmarks(bookmarkData.length === CHATS_PER_PAGE);
      }
      
      if (page === 1) {
        setInitialLoadComplete(true);
        setIsBookmarksLoaded(true);
        lastBookmarkLoadTimeRef.current = Date.now();
      }
      setIsBookmarksLoadingMore(false);
    } catch (error) {
  ...
  const loadChats = useCallback(async (page = 1, append = false, forceRefresh = false) => {
    ...
    if (!forceRefresh && page === 1 && !append && lastLoadedUserId === user.id && isCacheValid && isChatsLoaded) {
    ...
    try {
      ...
      if (append) {
        ...
        setHasMoreChats(sessions.length === CHATS_PER_PAGE)
      }
  ...
  useEffect(() => {
    // 현재 모드에 따른 상태 값 추출
    const hasMore = isBookmarkMode ? hasMoreBookmarks : hasMoreChats;
    const isLoadingMore = isBookmarkMode ? isBookmarksLoadingMore : isChatsLoadingMore;
    const isLoaded = isBookmarkMode ? isBookmarksLoaded : isChatsLoaded;
    const currentPage = isBookmarkMode ? currentBookmarkPage : currentChatPage;
    const loadFunc = isBookmarkMode ? loadBookmarksRef : loadChatsRef;

    // 검색 중이 아니고, 해당 모드의 첫 페이지가 로드되었으며, 더 가져올 데이터가 있는 경우에만 작동
    if (!searchTerm && hasMore && isLoaded) {
      const observer = new IntersectionObserver(
        (entries) => {
          if (entries[0].isIntersecting && !isLoadingMore) {
            loadFunc.current(currentPage + 1, true);
          }
        },
        { 
          threshold: 0.1,
  ...
  const handleRemoveBookmark = useCallback(async (bookmarkId: string, e: React.MouseEvent) => {
    if (isAnonymousUser) return;
    e.stopPropagation();
    e.preventDefault();
    ...
    try {
      ...
      setBookmarkSearchResults(prev => prev.filter(b => b.bookmark_id !== bookmarkId));
    } catch (error) {
      console.error('Error removing bookmark:', error);
    }
  }, [supabase, isAnonymousUser]);
  ...
  const handleSaveChatTitle = useCallback(async () => {
    ...
    try {
      ...
      // 북마크 모드일 때 북마크 리스트도 업데이트
      if (isBookmarkMode) {
        setBookmarks(prev => 
          prev.map(b => 
            b.id === editingChatId 
              ? { ...b, title: editingTitle.trim() }
              : b
          )
        )
        setBookmarkSearchResults(prev => 
          prev.map(b => 
            b.id === editingChatId 
              ? { ...b, title: editingTitle.trim() }
              : b
  ...
  }, [editingChatId, editingTitle, user, supabase, isAnonymousUser, isBookmarkMode])
  ...
  const searchBookmarks = useCallback(async (term: string) => {
    if (!user || !term.trim()) return;
    setIsSearching(true);
    try {
      const search = term.toLowerCase();
      
      // 1. 사용자의 북마크 목록을 먼저 가져옴
      const { data: userBookmarks, error: bookmarkError } = await supabase
        .from('message_bookmarks')
        .select('id, message_id, chat_session_id, created_at')
        .eq('user_id', user.id);
      ...
      const bookmarkedMessageIds = userBookmarks.map(b => b.message_id);

      // 2. 북마크된 메시지 중에서 검색어와 일치하는 메시지 검색
      const { data: matchingMessages, error: messageError } = await supabase
        .from('messages')
        .select('id, content, model, chat_session_id')
      ...
      const processedResults = await Promise.all(
        matchingMessages.map(async (msg) => {
          const bookmarkInfo = userBookmarks.find(b => b.message_id === msg.id);
          
          const { data: chatSession } = await supabase
            .from('chat_sessions')
            .select('title, current_model')
            .eq('id', msg.chat_session_id)
            .single();

          return {
            id: msg.chat_session_id,
            bookmark_id: bookmarkInfo?.id,
            message_id: msg.id,
            title: chatSession?.title || 'Untitled Chat',
            lastMessage: msg.content,
            lastMessageTime: bookmarkInfo ? new Date(bookmarkInfo.created_at).getTime() : Date.now(),
            current_model: chatSession?.current_model || msg.model,
            created_at: bookmarkInfo?.created_at || new Date().toISOString(),
            isBookmark: true
          };
        })
      );

      setBookmarkSearchCache(prev => new Map(prev).set(search, processedResults));
      setBookmarkSearchResults(processedResults);
    } catch (error) {
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      if (searchTerm.trim()) {
        ...
        if (isBookmarkMode) {
          if (bookmarkSearchCache.has(trimmedTerm)) {
            setBookmarkSearchResults(bookmarkSearchCache.get(trimmedTerm) || []);
            setIsSearching(false);
            return;
          }
  ...
  }, [searchTerm, searchCache, bookmarkSearchCache, isBookmarkMode]);

  const displayChats = useMemo(() => {
    if (isBookmarkMode) {
      return searchTerm.trim() ? bookmarkSearchResults : bookmarks;
    }
    return searchTerm.trim() ? searchResults : chats;
  }, [searchTerm, searchResults, bookmarkSearchResults, chats, bookmarks, isBookmarkMode]);
      ...
      <div className="h-full flex flex-col w-full overflow-x-hidden">
        <div ref={scrollContainerRef} className="flex-1 overflow-y-auto overflow-x-hidden sidebar-scroll w-full mt-0 md:mt-0 px-4 md:px-3 relative pb-24">
          <div className="px-2 py-2 pt-16 sm:pt-16">
            <h2 className="font-bold text-2xl">{isBookmarkMode ? translations.bookmarks : translations.messages}</h2>
          </div>
            ...
            <div className="space-y-3">
              ...
              {isAnonymousUser ? (
                <div className="px-4 py-6">
                        ...
                        <div className="flex items-center gap-3 w-full">
                            ...
                            <div className="w-10 h-10 rounded-full flex items-center justify-center bg-(--accent)/20">
                              {i === 1 ? (
                                <Image 
                                  ...
                                  width={20}
                                  height={20}
                                  className="object-contain text-(--muted)"
                                />
                              ) : i === 2 ? (
                                <Image 
                                  src="/logo/grok.svg"
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex justify-between items-baseline gap-1">
                              <p className="text-sm font-semibold truncate text-(--muted)" style={{ maxWidth: 'calc(100% - 0px)' }}>
                                {i === 1 ? 'Where do socks go after laundry?' : i === 2 ? 'How do I find Nemo?' : 'Why is my WiFi so slow?'}
                              </p>
                              <span className="text-xs shrink-0 text-(--muted)/60">
                                {i === 1 ? '2h' : i === 2 ? '1d' : '3d'}
                              </span>
                            </div>
                            <p className="text-xs truncate pr-2 text-(--muted)/60 mt-1">
                  ...
                  <div className="bg-linear-to-r from-(--accent)/10 to-(--accent)/5 rounded-xl p-4 border border-(--accent)/20">
                    <div className="text-center">
                      <div className="w-8 h-8 mx-auto mb-3 rounded-full bg-(--accent)/20 flex items-center justify-center">
                  ...
                  <div className="px-0 py-0">
                    {isSearching && (
                      <div className="flex flex-col items-center py-4 space-y-2">
                        <div className="w-6 h-6 border-2 border-t-transparent border-(--foreground) rounded-full animate-spin"></div>
                        <span className="text-xs text-(--muted)">Searching all conversations...</span>
                      </div>
                    )}
                    
                    {searchTerm && !isSearching && displayChats.length > 0 && (
                      <div className="px-2 py-1 text-xs text-(--muted) text-center">
                    ...
                    {!isSearching && displayChats.length > 0 ? (
                      <>
                            ...
                            <div key={`${chat.id}-${index}`} className={`last:border-b-0 flex items-center transition-all duration-200 ${isSelectionMode ? 'pl-2' : 'pl-0'} min-w-0 py-0`}>
                              {isSelectionMode && (
                                <div className="pr-3 cursor-pointer" onClick={() => handleSelectChat(chat.id)}>
                                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${selectedChatIds.includes(chat.id) ? 'bg-[#007AFF] border-[#007AFF]' : 'border-(--muted) opacity-50'}`}>
                              ...
                              <Link
                                href={editingChatId === chat.id || isSelectionMode ? '#' : (chat.isBookmark ? `/chat/${chat.id}?messageId=${chat.message_id}` : `/chat/${chat.id}`)}
                              ...
                              >
                                <div className="flex items-center gap-3 w-full">
                                  ...
                                  <div className="flex-1 min-w-0 overflow-hidden border-b border-(--sidebar-divider) pb-6 pt-3">
                                        ...
                                        <input
                                          ...
                                          className={`text-sm font-semibold bg-transparent border-b-2 outline-none w-full mr-2 ${ 
                                            isSelected 
                                              ...
                                              : 'text-(--foreground) placeholder-gray-400 border-gray-300 focus:border-(--foreground)'
                                          }`}
                                    ...
                                    <div className="flex justify-between items-end mt-0.5">
                                      ...
                                      <div className={`flex items-center gap-1 transition-opacity ${
                                        isSelected ? 'opacity-100' : (isBookmarkMode ? 'opacity-100' : 'opacity-0 group-hover:opacity-100')
                                      }`}>
                                        {isBookmarkMode ? (
                                          <button 
                                            onClick={(e) => handleRemoveBookmark(chat.bookmark_id, e)}
                                            className={`p-1 rounded-full transition-colors ${isSelected ? 'bg-white/20 hover:bg-white/30' : 'bg-(--accent) hover:bg-(--subtle-divider)'}`}
                                            title="Remove bookmark"
                                            type="button"
                                            aria-label="Remove bookmark"
                                          >
                                            <Bookmark 
                                              className={`w-3.5 h-3.5 ${isSelected ? 'text-white' : 'text-[#007AFF]'}`} 
                                              filled 
                                            />
                                          </button>
                                        ) : (
                        ...
                        {!searchTerm && (isBookmarkMode ? hasMoreBookmarks : hasMoreChats) && (
                          <div 
                            ref={loadMoreTriggerRef} 
                            className="flex justify-center py-6 min-h-[50px] w-full"
                          >
                            {(isBookmarkMode ? isBookmarksLoadingMore : isChatsLoadingMore) && (
                              <div className="w-6 h-6 border-2 border-t-transparent border-(--foreground) rounded-full animate-spin"></div>
                            )}
                          </div>
                        )}
                      ...
                      !isSearching && (
                        <div className="px-4 py-3 text-sm text-(--muted) text-center bg-(--accent)/5 rounded-lg">
          ...
          }, [displayChats, pathname, handleDeleteChat, handleRemoveBookmark, hasMoreChats, hasMoreBookmarks, isChatsLoadingMore, isBookmarksLoadingMore, currentTime, editingChatId, editingTitle, handleEditChatTitle, handleSaveChatTitle, handleChatTitleKeyDown, searchTerm, isSearching, isSelectionMode, selectedChatIds, handleSelectChat, isBookmarkMode])}

        </div>

        <div className="absolute bottom-3 left-0 right-0 p-3 flex flex-col space-y-3 text-(--muted) z-20 pointer-events-none">
          {isSelectionMode ? (
            ...
            <>
              <div className="px-0 pointer-events-auto">
                <div className="flex items-center gap-2">
                  <div className="relative flex-1">
                    <input
                      ...
                      className={`w-full px-4 py-2.5 text-sm rounded-full placeholder-(--muted) focus:outline-none transition-all ${
                  ...
                  <button
                    onClick={() => {
                      if (pathname === '/chat') {
                        ...
                        // Next.js App Router 캐싱 문제로 인해 하드 네비게이션 사용
                    ...
                    className="flex items-center justify-center w-10 h-10 rounded-full transition-all duration-300 cursor-pointer text-[var(--foreground)]"
                    style={{
                      ...getAdaptiveGlassStyleBlur(),
                      border: 'none',
                      ...getAdaptiveGlassBackgroundColor(),
                    ...
                    aria-label="New Chat"
                  >
                    <SquarePencil className="w-8 h-8 pt-0.5 pl-0.5" />
                  </button>
                </div>
              </div>
            </>
