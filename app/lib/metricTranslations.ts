...
const metricTranslations: Record<string, Translations> = {
  en: {
    names: {
      default: 'Default',
      tps: 'Speed',
...
export function getMetricTranslationsForUser(userLang?: string): Translations {
    if (typeof navigator === 'undefined' && !userLang) return metricTranslations.en;
  
    const lang = userLang || navigator.language.toLowerCase();
    const langCode = lang.split('-')[0]; // 'ko-KR' -> 'ko'
  
    return metricTranslations[langCode] || metricTranslations.en;
} 