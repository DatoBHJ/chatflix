...
export default function ProjectViewerHeader({
  ...
  const showVisibilityToggle = (!isGuest && !viewContext) || (viewContext === 'cabinet' && !isGuest)
  ...
  return (
    <>
      ...
      {/* Visibility Toggle - top left (hidden for guests, not shown when viewContext is set) */}
      {!isOverlayVisible && (isMobile ? showMobileButtons : true) && showVisibilityToggle && (
        <button 
          className={`absolute top-4 left-4 p-2 rounded-full text-white transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed z-10000 flex items-center justify-center ${isOverlayVisible ? 'z-10' : 'z-10000'}`}
          style={getAdaptiveGlassStyleBlur()}
      ...
      {!isOverlayVisible && totalSlides > 1 && (
        <div className="absolute top-8 left-1/2 -translate-x-1/2 z-10000 flex gap-1.5 px-3 py-1.5 rounded-full" style={getAdaptiveGlassStyleBlur()}>
          {totalSlides <= 3 ? (
            ...
            [0, 1, 2].map((dotIdx) => {
              ...
              else if (currentIndex === totalSlides - 1) isActive = dotIdx === 2;
